...
async function createUser(user) {
    ...
    try {
        ...
        const insertNewUserId = await connection.query(`insert into "User" (id, email, password, name, phonenumber,
                                                                          createat,
                                                                          modifiedat, username)
                                                      values (DEFAULT,
                                                              '${user.email}',
                                                              ...
                                                              '${user.username}')
                                                      returning id`);
        const insertIndex = insertNewUserId.rows[0].id;
        const insertUserAddress = await connection.query(`insert into "UserAddress"
                                                        values (DEFAULT,
                                                                ${insertIndex},
                                                                '',
                                                                '')`);
...
async function getUser(token) {
    ...
    const connection = await new pg_1.Pool(posgre_1.PostgreSQLConfig);
    const result = await connection.query(`select "User".id,"User".username,"User".name,"User".email,"User".phoneNumber,"User".createAt,"UserAddress".address from "User" left join "UserAddress" on "User".id::text = "UserAddress".userId where "User".id = ${user.id};`);
...
async function updateUserInfo(oldId, user) {
    ...
    try {
        await connection.query("begin");
        const result = await connection.query(`update "User"
                                            set name        = '${user.name}',
...
async function updateUserAddress(id, userAddress) {
    ...
    try {
        await connection.query("begin");
        const result = await connection.query(`update "UserAddress"
...
async function getUserLoginInfo(username, password, type, token_device) {
    ...
    switch (type) {
        case "email": {
            const sqlQuery = `Select id, username, password, active
                            from "User"
...
async function updateUserPassword(id, oldPassword, newPassword) {
    const connection = await new pg_1.Pool(posgre_1.PostgreSQLConfig);
    try {
        const encryptedOldPassword = (0, md5_1.default)(oldPassword);
        const encryptedNewPassword = (0, md5_1.default)(newPassword);
        await connection.query("begin");
...
async function getUserTokenDevice(userId) {
    try {
        const connection = await new pg_1.Pool(posgre_1.PostgreSQLConfig);
        const result = await connection.query(`select tokendevice as "tokenDevice"
                                            from "User"
                                            where id = ${userId}`);
...
async function checkActiveStatus(userId) {
    const connection = await new pg_1.Pool(posgre_1.PostgreSQLConfig);
    try {
        const result = await connection.query(`select active
                                          from "User"
                                          where id = ${userId}`);
